<template>
  <Teleport to="body">
    <!-- Mobile: full width — Desktop: pojok kanan atas -->
    <div class="fixed z-[9999] flex flex-col gap-3 pointer-events-none
                top-4 left-4 right-4
                sm:top-6 sm:left-auto sm:right-6 sm:w-80">
      <TransitionGroup name="toast">
        <div
          v-for="toast in toasts"
          :key="toast.id"
          class="pointer-events-auto flex items-start gap-3 px-4 py-3.5 rounded-xl shadow-lg w-full"
...
function toastIcon(type) {
  const map = { success: '✅', error: '❌', info: 'ℹ️', warning: '⚠️' }
  return map[type] ?? 'ℹ️'
}
</script>
