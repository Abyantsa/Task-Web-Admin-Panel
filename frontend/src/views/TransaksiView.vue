    ...
    >
      ...
      <div class="flex-1 p-4 sm:p-6 lg:p-8">
        ...
        <div class="mb-5 lg:mb-8">
          <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Transaksi</h2>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-6">
          ...
          <form @submit.prevent="submitTransaksi" class="space-y-4">
            <div class="flex flex-col sm:flex-row gap-4">
              <div class="flex-1">
                ...
                >
                  ...
                  <option v-for="p in produkList" :key="p.id" :value="p.id" :disabled="p.stok === 0">
            ...
            <!-- Preview total — tampil di semua ukuran -->
            <div v-if="previewTotal" class="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-2.5">
              <span class="text-sm text-emerald-700 font-medium">Total</span>
              <span class="font-bold text-emerald-700 text-base">{{ formatRupiah(previewTotal) }}</span>
            </div>

            <button
              type="submit"
              class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2.5 rounded-lg text-sm transition-colors disabled:opacity-60"
              :disabled="submitting"
            >
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
          ...
          <div v-else>
            ...
            <div class="lg:hidden divide-y divide-gray-100">
              <div
                v-for="(t, i) in paginatedList"
                :key="t.id"
                class="px-4 py-4 hover:bg-gray-50 transition-colors"
              >
                <div class="flex items-start justify-between gap-3 mb-1.5">
                  <p class="font-semibold text-gray-800 text-sm leading-snug flex-1">{{ t.nama_produk }}</p>
                ...
                <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-gray-400">
                  ...
                  <span>·</span>
                  <span>{{ formatRupiah(t.harga_satuan) }} × {{ t.qty }}</span>
                  <span>·</span>
                  <span class="flex items-center gap-1">
                    <span class="w-4 h-4 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center uppercase text-xs">{{ t.username?.charAt(0) }}</span>
                    {{ t.username }}
                  </span>
                </div>
              </div>
                  ...
                  <tr v-for="(t, i) in paginatedList" :key="t.id" class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                      <span class="inline-flex items-center gap-1 text-xs text-gray-600">
                        <span class="w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center uppercase text-xs">{{ t.username?.charAt(0) }}</span>
                        {{ t.username }}
                      </span>
                    </td>
                  </tr>
            ...
            <div class="px-4 sm:px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
              <div class="flex items-center gap-2 sm:gap-3">
                <p class="text-xs text-gray-400 hidden sm:block">{{ startIndex }}–{{ endIndex }} dari {{ transaksiList.length }}</p>
                <select v-model.number="perPage" class="text-xs border border-gray-200 rounded-lg px-2 py-1.5 text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
import { PanelLeftOpen, PanelLeftClose, Menu } from '@lucide/vue'
...
const { currentPage, perPage, totalPages, paginatedList, startIndex, endIndex, pageNumbers, goTo, prev, next } = usePagination(transaksiList, 10)
...
async function submitTransaksi() {
  ...
  try {
    await api.post('/transactions', { produk_id: form.value.produk_id, qty: form.value.qty })
